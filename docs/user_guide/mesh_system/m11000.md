[<<< Back to Mesh Wi-Fi System](index.md)

# M11000

## Overview
- View the Mesh's appearance — [front](overview_m11000.md#front-panel){ data-preview } and [back](overview_m11000.md#back-panel){ data-preview }.
- Check the [LED](overview_m11000.md#front-panel){ data-preview } status and indication.
- Check the [interfaces](overview_m11000.md#back-panel){ data-preview } layout and instructions. 

## Quick Installation Guide
- [Scenario 1](qig.md#scenario-1-set-up-main-unit-for-a-new-mesh-network){ data-preview }: Set up main unit for a New Mesh Network.
- [Scenario 2](qig.md#scenario-2-add-a-new-unit-to-an-existing-mesh-network){ data-preview }: Add a new unit to an existing Mesh Network.
...
- *Status* shows whether the Mesh router has connected to the Internet or not, and its work mode and interface. Click [*Quick Setup*](qig.md){ data-preview } to redirect to the *Quick Setup* page.

- *Mesh* shows the Mesh router’s Mesh network status (SOLE or MESH), the mesh device name and the amount of mesh units. Click [*More Details*](mesh.md){ data-preview } to know more information or add/manage mesh unit(s).

- *Devices* shows the number of devices connected to this Mesh router, via Ethernet cable or 2.4G WiFi or 5G WiFi. Click [*More Details*](devices.md){ data-preview } to know more information about these devices or configure the settings.
...
- *Wireless 2.4G* shows the Wireless 2.4G status. Click [*More Details*](wireless.md#24g5g){ data-preview } to know more information about it or make changes on Settings page. 

- *Wireless 5G* shows the Wireless 5G status. Click [*More Details*](wireless.md#24g5g){ data-preview } to know more information about it or make changes on Settings page. 

- *Wireless 6G* shows the Wireless 6G status. Click [*More Details*](wireless.md#6g){ data-preview } to know more information about it or make changes on Settings page. 
...
- [WISP](wisp.md){ data-preview }: Enable the Mesh router to wirelessly connect to a distant Wi-Fi network and share that connection locally via Ethernet/Wi-Fi, acting as a client bridge.
- [Wireless](wireless.md){ data-preview }: is to customize the Mesh router's wireless settings, including SSID and password, mode, channel, channel width and transmit power, and so on.
...
- [Firmware](firmware.md){ data-preview }: Update the Mesh router's firmware to be the latest version.
...
- [LAN](network.md#lan){ data-preview }: Modify the Mesh router's LAN IP address, when necessary.
...
- [DHCP Server](network.md#dhcp-server){ data-preview }: Specify the IP address for client devices by the Mesh router as a DHCP Server.
...
- [WiFi Schedule](security.md#wifi-schedule){ data-preview }: Turn on/off the Mesh router's wireless network automatically at a specific time.
...
- [System Time](system.md#system-time){ data-preview }: Configure the Mesh router's system time, necessary for some time-based functions.
...
- [Administration](system.md#administration){ data-preview }: Access and manage the Mesh router from the local or remote devices.
- [Admin Account](system.md#admin-account){ data-preview }: Change the Mesh router's login password.
- [Language](system.md#language){ data-preview }: Customize the Mesh router's web management language.
- [Timed Reboot](system.md#timed-reboot){ data-preview }: Schedule for the Mesh router rebooting.
- [Reboot](system.md#reboot){ data-preview }: Reboot the Mesh router for the settings or changes to take effect.
- [Reset](system.md#reset){ data-preview }: Restore the Mesh router to its factory default settings.
- [LED Control](system.md#led-control){ data-preview }: Control the Mesh router's LED.
- [TR069](system.md#tr069){ data-preview }: Configure the Mesh router's TR069 (CPE WAN Management).
...
- [PING](diagnostic_tools.md#ping){ data-preview }: Used to test the connectivity between the Mesh router and the tested host, and measure the round-trip time. 
- [TRACEROUTE](diagnostic_tools.md#traceroute){ data-preview }: Used to test the route (path) your Mesh router has passed to reach the tested host, and measure transit delays of packets across an Internet Protocol network. 
- [NSLOOKUP](diagnostic_tools.md#nslookup){ data-preview }: Check if the DNS IP address of the WAN can work normally. 
- [System Log](diagnostic_tools.md#system-log){ data-preview }: Track all the Mesh router behaviors. When the Mesh router does not work normally, download the system log and send it to our Technical Support for troubleshooting.
